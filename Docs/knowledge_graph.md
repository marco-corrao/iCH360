...
### Reaction nodes
Nodes of type `reaction` represent chemical or biochemical reactions in the model and their ID maps directly to the BIGG idetifiers of the reactions in the COBRA model. As such, they are the the primary links connecting the metabolic model and the knowledge graph.
### Protein nodes
Nodes of type `protein` represent all biological proteins and are uniquely mapped with the EcoCyc database. All protein nodes are annotated with a molecular weight (in kDa), which was computed recursively based on known polypeptide weights. Protein nodes are further catalogued into subtypes, namely
...
- If the node is a reaction, its GPR is first constructed as the OR of the GPRs of its catalytic children (nodes connected to it via a `catalyis` edge, excluding edge `inactive` subtypes and, depending on application, `secondary` ones ). To account for non-catalytic protein requirements of the reaction this expression is then ANDed with the GPR expressions of all non-catalytic children (nodes connected to it via a `non_catalytic_requirement` edge).
